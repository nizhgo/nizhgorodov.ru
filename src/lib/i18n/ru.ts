...
export const ru: Translations = {
	...
	about: {
		...
		body: [
			...
			'Работал по-разному: где-то вёл команду фронтендеров — ревью, менторинг, архитектура с нуля; где-то был единственным фронтом и закрывал весь фронт продукта.',
			'Закончил магистратуру ВШЭ по цифровой урбанистике — геоданные, Python, QGIS. Открыт к релокейту и удалёнке, беру фриланс.'
		]
	},
	ui: {
		status: 'Статус',
		...
		items: [
			...
			},
			{
				...
				bullets: [
					...
					'Legacy-чат надо было собрать под одну крышу и довести до прода — сам поднял деплой (Docker + nginx) и встроил отдельное Next.js-приложение чата через iframe с мостом Telegram↔iframe: safe-area и клавиатура через postMessage, проброс haptic. Один деплой, чат живёт прямо внутри хаба.'
				],
			...
			},
			{
				...
				bullets: [
					...
					'Медданные нельзя просто прятать в UI — развёл их по отдельным таблицам и сделал RBAC на уровне чтения: урезанная роль физически не джойнится к медицинским полям. Чувствительные данные режутся на бэкенде, до клиента они вообще не доходят.',
